...
import { Modal, View, Button, Image, Text, TouchableOpacity, ActivityIndicator, StyleSheet, ScrollView } from 'react-native';
  ...
  return (
    <Modal visible={visible} animationType="slide">
      <View style={estilosModalFormularioPublicaciones.container}>
        {/* Header */}
        <HeaderFormulario titulo={esEdicion ? 'Editar Publicación' : 'Nueva Publicación'} evento={handleCancel} 
        ...
        <ScrollView style={estilosModalFormularioPublicaciones.formContent}>
          ...
          <InputFormulario control={control} name="titulo" rules={validaciones.titulo_publicacion} icono={{nombre:'pricetag-outline',color:'#95a5a6'}} error={errors.titulo} placeholder="Ingresa el título de la publicación" />
          <ErrorFormulario error={errors.titulo} />

          {/* Descripción */}       
          <LabelInput titulo="Descripción" icono={{nombre:'document-text-outline',color:'#3498db'}} />
          <InputFormulario control={control} name="descripcion" rules={validaciones.descripcion_publicacion} icono={{nombre:'reader-outline',color:'#95a5a6'}} error={errors.descripcion} placeholder="Describe tu publicación" estilos={estilosModalFormularioPublicaciones.textArea} />
